...
function quiz1(x?:string) : string{
  if ( typeof(x) == 'string'){
    return '안녕하세요' + x
  } else {
    return '이름이 없습니다.'
...
function quiz2(x : string | number) : number{
  if ( typeof(x) == "string" ){
    return (x.length)
  }
  if ( typeof(x) == "number" ){
...
function quiz3(월소득:number, 집보유여부:boolean, 매력:string): string {
  let score = 0; 
  let 월소득점수 = 월소득;
  let 집보유여부점수 = 0;
  let 매력점수 = 0;
